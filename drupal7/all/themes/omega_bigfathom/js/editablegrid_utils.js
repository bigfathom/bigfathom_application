...
EditableGrid.prototype.sort_sortablehtml = function(a,b) 
{
    var getSortValue = function(raw)
    {
        var endIdx = raw.indexOf("]", 9);
        if(endIdx < 0) return -1;
        return raw.substring(9,endIdx);
    };
    
    if(a[0] && b[0] && a[0].length > 8 && b[0].length > 8)
    {
        if(a[0].substring(0,9) === "[SORTNUM:" && b[0].substring(0,9) === "[SORTNUM:")
        {
            var aN = [getSortValue(a[0])];
            var bN = [getSortValue(b[0])];
            return EditableGrid.prototype.sort_numeric(aN,bN);
        } else
        if(a[0].substring(0,9) === "[SORTSTR:" && b[0].substring(0,9) === "[SORTSTR:")
        {
            var aS = [getSortValue(a[0])];
            var bS = [getSortValue(b[0])];
            return EditableGrid.prototype.sort_alpha(aS,bS);
        }
    }
    return EditableGrid.prototype.sort_alpha(a,b);
};
...
EditableGrid.prototype.checkDate = function(strDate, strDatestyle) 
{
	...
	strDatestyle = strDatestyle || "ISO8601";
	...
	if (booFound == false) 
		...
		strYear = strDate.substr(4);
                ...
                console.log("Unrecognized date format '" + strDatestyle + "' for  value=" + strDate);
                return 1;
            }          
	}
