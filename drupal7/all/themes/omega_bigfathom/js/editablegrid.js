/**
 * Forked by Room4me.com Software LLC 20160619
 */
 ...
 * <li>dateFormat: ISO8601 or EU or US (default="ISO6601")</li>
...
{ 
    if (name) 
    {
        ...
        throw "Missing name on constructor EditableGrid!";
    }
}
...
EditableGrid.prototype.dateFormat = "ISO8601";
...
EditableGrid.prototype.init = function (name, config)
{
	if (typeof name != "string" || (typeof config != "object" && typeof config != "undefined")) 
		...
		alert("The EditableGrid constructor takes two arguments:\n- name (string)\n- config (object)\n\nGot instead " + (typeof name) + " and " + (typeof config) + ".");
                throw "Failed to initialize EditableGrid!";
	};
...
EditableGrid.prototype.cellClicked = function(rowIndex, columnIndex) {};    //Created 20170607
/** Notifies that a row has been deleted */
EditableGrid.prototype.rowRemoved = function(oldRowIndex, rowId) {};


/**
 * Provide visual indicator that a specific cell can be edited
 * Also unlock if locked
 */
EditableGrid.prototype.markCellEditable = function(rowIndex, columnIndex)
{
    var element = this.getCell(rowIndex, columnIndex);
    if(element)
    {
        if(EditableGrid.prototype.hasClassName(element, "locked"))
        {
            EditableGrid.prototype.removeClassName(element, "locked");
        }
        if(EditableGrid.prototype.hasClassName(element, "notapplicable"))
        {
            EditableGrid.prototype.removeClassName(element, "notapplicable");
        }
        EditableGrid.prototype.addClassName(element, "canedit");
...
/**
 * Provide visual indicator that a specific cell is locked from editing
 * Also lock if unlocked
 */
EditableGrid.prototype.markCellLocked = function(rowIndex, columnIndex)
{
    var element = this.getCell(rowIndex, columnIndex);
    if(element)
    {
        if(EditableGrid.prototype.hasClassName(element, "canedit"))
        {
            EditableGrid.prototype.removeClassName(element, "canedit");
        }
        EditableGrid.prototype.addClassName(element, "locked");    
        var column = this.getColumn(columnIndex);
        if(column.datatype === "boolean")
        {
...
/**
 * Provide visual indicator that a specific cell has no applicable data
 */
EditableGrid.prototype.markCellNotApplicable = function(rowIndex, columnIndex)
{
    var element = this.getCell(rowIndex, columnIndex);
    if(element)
    {
        ...
        EditableGrid.prototype.addClassName(element, "notapplicable");    
        var column = this.getColumn(columnIndex);
        if(column.datatype === "boolean")
        {
            //Disable the checkbox
...
/**
 * Provide visual indicator that a specific cell's data is applicable to the row
 */
EditableGrid.prototype.markCellApplicable = function(rowIndex, columnIndex, is_editable)
{
...
EditableGrid.prototype.processXML = function()
{
	with (this) {
		...
		for (var i = 0; i < rows.length; i++) 
		{
			...
			// for each row we keep the original index, the id and all other attributes that may have been set in the XML
...
EditableGrid.prototype.attachToHTMLTable = function(_table, _columns)
{
	...
	for (var i = 0; i < rows.length; i++) {
            ...
            var thisrow = { visible: true, originalIndex: i, id: rows[i].id, columns: rowData };
            for (var attrIndex = 0; attrIndex < rows[i].attributes.length; attrIndex++) 
            {
                var node = rows[i].attributes.item(attrIndex);
                if (node.name.toLowerCase() !== "id")
                {
//alert("Look row#" + i + " has " + node.name + " = " + node.value);                  
                    thisrow[node.nodeName] = node.value;
                } 
            }            
            this.data.push(thisrow);
            rows[i].rowId = rows[i].id;
            rows[i].id = this._getRowDOMId(rows[i].id);
            /*
            var rowattribs = rows[i].attributes;
            //Now add all the custom row attributes too
            for (var a = 0; a < rowattribs.length; a++)
            {
                var attrib = rowattribs[a];
                if(attrib.nodeName.toLowerCase() !== 'id')
                {
                    rows[i][attrib.nodeName] = attrib.value;   
                }
            }
        */
...
/**
 * Remove all the rows in the table (added by FJF 20161209)
 * The callback passes null parameters.
 * @param {Integer} rowIndex
 * @returns {Number} number of rows removed
 */
EditableGrid.prototype.removeAllRows = function()
{
    var rowcounttoremove = this.data.length;
    for (var rowIndex = 0; rowIndex < this.data.length; rowIndex++) if (this.data[rowIndex].id == rowId) return rowIndex;    
    for (var rowIndex = this.data.length-1; rowIndex >= 0; rowIndex--)
    {
    ...
    // callback
    this.rowRemoved(null,null);
    
    // refresh grid
    this.refreshGrid();
    
    return rowcounttoremove;
};
...
EditableGrid.prototype._renderOneHeaderRow = function(cols) 
{
	with (this) 
        {
            var columnIndexInModel = 0;
...
EditableGrid.prototype._renderHeaders = function() 
{
	with (this) 
		...
		var rows = tHead.rows;
                ...
                var cols = rows[lastrow].cells;
                _renderOneHeaderRow(cols);
                /*
...
EditableGrid.prototype.mouseClicked = function(e) 
{
	...
	with (this) 
		...
		var columnIndex = target.cellIndex;
                ...
                //Bubble up that this cell was clicked
...
EditableGrid.prototype.sort = function(columnIndexOrName, descending, backOnFirstPage)
{
	with (this) {
		...
		if (typeof backOnFirstPage  == 'undefined') backOnFirstPage = true;
		...
		var sort_function = type == "integer" || type == "double" || type == "numid" ? sort_numeric : type == "boolean" ? sort_boolean : type == "date" ? sort_date : (type == "sortablehtml" || type == "formula") ? sort_sortablehtml : sort_alpha;
 ...
 * @param {Array} cols Columns to filter on.  If cols is not specified, the filter will be done on all columns
 */
EditableGrid.prototype.filter = function(filterString, cols, customRowMatchFunction)
{
	with (this) 
        {

                var limited_cols = (typeof cols != 'undefined' && cols !== null);
                var call_crff = (typeof customRowMatchFunction != 'undefined' && customRowMatchFunction != null);
		...
		var rowCount = getRowCount();
		var columnCount = (typeof cols != 'undefined' && cols !== null) ? cols.length  : getColumnCount();
		...
		for (var r = 0; r < rowCount; r++) 
			...
			row.visible = true;
			var rowContent = ""; 
                        var pass_custom_filter;
                        if(!call_crff)
                        {
                            pass_custom_filter = true;
                        } else {
                            pass_custom_filter = customRowMatchFunction(this, row);    
                        }

                        if(!pass_custom_filter)
                        {
			...
			for (var c = 0; c < columnCount; c++) 
				...
				var displayValue = getDisplayValueAt(r, (typeof cols != 'undefined' && cols !== null)  ? cols[c] :  c);
				var value = getValueAt(r, (typeof cols != 'undefined' && cols !== null)  ? cols[c] : c);
			...
			for (var i = 0; i < words.length; i++) 
				...
				if (!word.endsWith("!")) {
					...
					if (colindex >= 0) match = (''+getDisplayValueAt(r, colindex)).trim().toLowerCase() == word || (''+getValueAt(r, colindex)).trim().toLowerCase() == word; 
						...
						if (getColumnType((typeof cols != 'undefined' && cols !== null)  ? cols[c] : c) == 'boolean') continue;
						if ((''+getDisplayValueAt(r, (typeof cols != 'undefined' && cols !== null) ? cols[c] : c)).trim().toLowerCase() == word || (''+getValueAt(r, (typeof cols != 'undefined' && cols !== null)  ? cols[c] : c)).trim().toLowerCase() == word) match = true;
					}
				}
