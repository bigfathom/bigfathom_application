 ...
 * 
 * Forked by Room4me.com Software LLC 20160621
 * 
 * @constructor
...
//20160621 FJF - Removed escape check because redundant
CellRenderer.prototype._render = function(rowIndex, columnIndex, element, value) 
{
	...
	// clear isEditing (in case a currently edited is being re-rendered by some external call)
...
//20160621 FJF - Make escapehtml optional param and support sortablehtml
//20160622 FJF - Added support for formula datatype
CellRenderer.prototype.render = function(element, value, escapehtml) 
{
    var _value;
    if(this.column.datatype === "formula")
    {
        if(value.length > 8 && value.substring(0,5) === "[SORT")
        {
            var endIdx = value.indexOf("]", 9);
            if(endIdx > 0)
            {
                 //Strip out the sorting instruction
                _value = String(value.substring(endIdx+1));
            } else {
                _value = "ERR SYNTAX-" + String(value);
            }
        } else {
            _value = String(value);
        }
        ...
        {
            _value = escapehtml ? (typeof value == 'string' && this.column.datatype != "html" && this.column.datatype != "sortablehtml" ? (value === null ? null : htmlspecialchars(value, 'ENT_NOQUOTES').replace(/  /g, ' &nbsp;')) : value) : value;
        } else {
            if(!escapehtml)
            {
                _value = value;
            } else {
                _value = (value === null ? null : htmlspecialchars(value, 'ENT_NOQUOTES').replace(/  /g, ' &nbsp;'));
            }
        }
    }
