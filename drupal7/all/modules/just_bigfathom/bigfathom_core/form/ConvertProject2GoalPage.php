...
/**
 * Convert a goal into a project.  By default, the project is saved as
 * an antecedent of the goal's owner project.
 * 
 * @author Frank Font
 */
class ConvertProject2GoalPage extends \bigfathom\ASimpleFormPage
{
    ...
    public function __construct($urls_override_arr=NULL, $parent_projectid=NULL, $this_projectid=NULL)
    {
        if(empty($parent_projectid))
        {
            throw new \Exception("Missing required dependent_projectid parameter!");
        }
        if(empty($this_projectid))
        {
            throw new \Exception("Missing required this_projectid parameter!");
        }
        $this->m_dependent_projectid = $parent_projectid;
        ...
        $this->m_oPageHelper = new \bigfathom\ConversionPageHelper($urls_override_arr,NULL,$this->m_dependent_projectid,$this->m_this_projectid,NULL);
    ...
    function updateDatabase($form, $myvalues)
    {
        ...
        try
        {
            ...
            $old_projectid = $myvalues['this_projectid'];
            $new_owner_projectid = $myvalues['parent_projectid'];
            
            $this->m_oWriteHelper->changeProjectOfGoalBranch($root_goalid, $new_owner_projectid);
            $this->m_oWriteHelper->deactivateProject($old_projectid);
            
            //If we are here then we had success.
            $msg = 'Discardeded project wrapper of goal#' . $root_goalid;
            drupal_set_message($msg);
        }
        catch(\Exception $ex)
        {
            $msg = t("Failed because " . $ex->getMessage());
            error_log("$msg\n" 
    ...
    function getForm($form, &$form_state, $disabled, $myvalues_override)
    {
        ...
        $new_form = $this->m_oPageHelper->getProject2GoalForm('A',$form, $form_state, $disabled, $myvalues, $html_classname_overrides);
        ...
        $new_form['data_entry_area1']['action_buttons']['create'] = array('#type' => 'submit'
                ...
                , '#value' => t('Convert this Project into a Goal'));
