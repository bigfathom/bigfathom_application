...
/**
 * This class interacts with project level communications
 *
 * @author Frank Font of Room4me.com Software LLC
 */
class ManageProjectCommentsPage extends \bigfathom\ASimpleFormPage {
    ...
    public function __construct($projectid=NULL, $urls_override_arr=NULL)
    {
        ...
        if(empty($projectid))
        {
            throw new \Exception("Cannot find comments without a projectid!");
        }
        ...
        $urls_arr = [];
        $urls_arr['add'] = 'bigfathom/project/addcomment';
        $urls_arr['reply'] = 'bigfathom/project/addcomment';
        $urls_arr['edit'] = 'bigfathom/project/editcomment';
        $urls_arr['view'] = 'bigfathom/project/viewcomment';
        $urls_arr['delete'] = 'bigfathom/project/deletecomment';
        ...
        $this->m_oPageHelper = new \bigfathom\ProjectPageHelper($urls_arr, NULL, $this->m_parent_projectid);
    ...
    function getForm($form, &$form_state, $disabled, $myvalues, $html_classname_overrides = NULL) {
        try 
        {
            $main_tablename = 'projectcomment-table';
            ...
            $dashboard = $this->m_oPageHelper->getContextDashboardElements($this->m_projectid);
            ...
            //$projects_lookup = $this->m_oPageHelper->getProjectOptions('',FALSE,FALSE);
            $rows = "\n";
            
            $only_active = TRUE;
            $only_in_tree = FALSE;
            //$all = $this->m_oMapHelper->getProjectsInProjectByID($this->m_parent_projectid, $only_active, $only_in_tree);
            $project_status_by_code = $this->m_oMapHelper->getWorkitemStatusByCode();
            $action_status_by_code = $this->m_oMapHelper->getActionStatusByCode();

            $projectid=$this->m_projectid;
            $allcommentsmap = $this->m_oMapHelper->getProjectCommunications($projectid);
            ...
            foreach ($naturalorder_ar as $comid) 
            {
                ...
                {
                    $status_record = $project_status_by_code[$status_cd_at_time_of_com];
            ...
            $form["data_entry_area1"]['table_container']['maintable'] = array('#type' => 'item',
                ...
                . '<th><span title="Project status at time of comment creation">' . t('Project Status') . '</span></th>'
            ...
            if (isset($this->m_urls_arr['add']) && $this->m_parent_projectid != NULL)
            {
                if (strpos($this->m_aPersonRights, 'A') !== FALSE) {
                    $add_link_markup = l('Add New Project Comment Thread'
                            , $this->m_urls_arr['add']
                            , array('query' => array('projectid' => $this->m_projectid),
