 ...
 */
class ManageSprintCommentsPage extends \bigfathom\ASimpleFormPage {
    ...
    private $m_custom_page_key = NULL;
    ...
    public function __construct($sprintid=NULL, $urls_override_arr=NULL)
    {
        ...
        if(empty($sprintid))
        {
            throw new \Exception("Cannot find comments without a sprintid!");
        }
        ...
        $urls_arr = array();
        $urls_arr['add'] = 'bigfathom/sprint/addcomment';
        $urls_arr['reply'] = 'bigfathom/sprint/addcomment';
        $urls_arr['edit'] = 'bigfathom/sprint/editcomment';
        $urls_arr['view'] = 'bigfathom/sprint/viewcomment';
        $urls_arr['delete'] = 'bigfathom/sprint/deletecomment';
        ...
        if(is_array($urls_override_arr) && !empty($urls_override_arr['cpk']))
        {
            ...
            $this->m_custom_page_key = "MSCP" . time();
        }
        if(!empty($urls_override_arr['return']))
        {
            ...
            //$dar = array('$urls_override_arr'=>$urls_override_arr,'$baseline_mi_info'=>$baseline_mi_info);
            //DebugHelper::showNeatMarkup($dar, "LOOK CPK={$this->m_custom_page_key} it was empty");
        } else {
            ...
            //$dar = array('$urls_override_arr'=>$urls_override_arr,'$urls_arr'=>$urls_arr,'$mi'=>$mi,'$pmi'=>$pmi);
            //DebugHelper::showNeatMarkup($dar, "LOOK CPK={$this->m_custom_page_key} it was empty now it is {$urls_arr['return']}");
        }
        ...
        $this->m_oPageHelper = new \bigfathom\SprintPageHelper($urls_arr, NULL, $this->m_parent_projectid);
    ...
    function getForm($form, &$form_state, $disabled, $myvalues, $html_classname_overrides = NULL) {
        try 
        {
            $main_tablename = 'sprintcomment-table';
            ...
            $dashboard = $this->m_oPageHelper->getContextDashboardElements($this->m_sprintid);
            ...
            $allcommentsmap = $this->m_oMapHelper->getSprintCommunications($sprintid);
            ...
            foreach ($naturalorder_ar as $comid) 
            {
                ...
                {
                    $status_record = $sprint_status_by_code[$status_cd_at_time_of_com];
            ...
            $form["data_entry_area1"]['table_container']['maintable'] = array('#type' => 'item',
                ...
                . '<th><span title="Sprint status at time of comment creation">' . t('Sprint Status') . '</span></th>'
            ...
            if (isset($this->m_urls_arr['add']) && $this->m_parent_projectid != NULL)
            {
                if (strpos($this->m_aPersonRights, 'A') !== FALSE) {
                    $add_link_markup = l('Add New Sprint Comment Thread'
                            , $this->m_urls_arr['add']
                            , array('query' => array('sprintid' => $this->m_sprintid,'cpk'=>$this->m_custom_page_key),
