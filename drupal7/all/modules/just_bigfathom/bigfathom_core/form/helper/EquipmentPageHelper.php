...
class EquipmentPageHelper
{
    ...
    public function __construct($urls_arr, $my_classname=NULL)
    {
        try
        {
            ...
            $this->m_equipment_tablename = DatabaseNamesHelper::$m_equipment_tablename;
     ...
     */
    function getFieldValues($equipmentid=NULL)
    {
        try
        {
            $myvalues = array();
            ...
            {
                $myvalues = $this->m_oMapHelper->getEquipmentByID($equipmentid);
            } else {
    ...
    function formIsValid($form, &$myvalues, $formMode)
    {
        try
        {
                ...
                if(!isset($myvalues['id']))
                {
                    ...
                    if($connection_infobundle['critical_connections_found'])
                    {
                        ...
                        form_set_error('shortname',"Cannot delete because critical connections were found. "
                                . "$help_detail  Consider retiring this resource instead of removing it.");
                        $bGood = FALSE;
                    }
                }
            }
            ...
            {
                form_set_error('shortname','The equipment name cannot be empty');
                $bGood = FALSE;
            } else {
                if($formMode == 'A' || $formMode == 'E')
                {
                    //Check for duplicate keys too
                    $result = db_select($this->m_equipment_tablename,'p')
                        ->fields('p')
                    ...
                    if($result->rowCount() > 0)
                    {
                        ...
                        {
                            form_set_error('shortname', 'Already have a equipment with this name');
                            $bGood = FALSE;
                        }
                    }
                }
    ...
    public function getEquipmentConditionOptions($show_terminal_text=TRUE)
    {
        try
        {
            //Get all the relevant select options
            $myoptions = array();
            $all = $this->m_oMapHelper->getEquipmentConditionByCode();
    ...
    function getForm($formType, $form, &$form_state, $disabled, $myvalues, $html_classname_overrides=NULL)
    {
        try
        {
            ...
            $thelimit = BIGFATHOM_MAX_EQUIPMENT_IN_SYSTEM;
            if($formType=='A' && $this->m_oMapHelper->getCountEquipment() >= $thelimit)
            {
                drupal_set_message("Cannot add another non-human resource because your system already has the configuration allowed limit of $thelimit",'error');
                $disabled = TRUE;
            }
            ...
            $options_condition = $this->getEquipmentConditionOptions();
            ...
            if($formType == 'D')
            {
                ...
                {
                    drupal_set_message("This resource cannot be deleted because it is in use by $critical_connections_found entities of the application.  Consider marking it retired instead.","warning");
                }
            }
            ...
            $form['data_entry_area1']['shortname'] = array(
                ...
                '#maxlength' => 40,
                '#required' => TRUE,
                '#description' => t('The unique short name for the equipment'),
                '#disabled' => $disabled
            );
            ...
            $form['data_entry_area1']['name'] = array(
                ...
                '#maxlength' => 80,
                '#required' => TRUE,
                '#description' => t('Descriptive full name of the equipment'),
                '#disabled' => $disabled
            );
            ...
            $form['data_entry_area1']['condition_cd'] = array(
                ...
                '#required' => FALSE,
                '#description' => t('Condition of this equipment'),
                '#disabled' => $disabled
            );
            ...
            $form['data_entry_area1']['active_yn'] = array(
                ...
                '#options' => $ynoptions,
                '#description' => t('Yes if equipment is available for use, else no.')
            );
