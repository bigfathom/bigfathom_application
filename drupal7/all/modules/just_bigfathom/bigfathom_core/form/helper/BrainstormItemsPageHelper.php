 ...
 */
class BrainstormItemsPageHelper
{
    ...
    function getFieldValues($brainstorm_id='')
    {
        try
        {
            if($brainstorm_id != NULL)
            {
                ...
                $myvalues = db_select(DatabaseNamesHelper::$m_brainstorm_item_tablename, 'n')
                  ->fields('n')
    ...
    public function formIsValid($form, &$myvalues, $formMode)
    {
        try
        {
            ...
            if(trim($myvalues['item_nm']) == '')
            {
                ...
                {
                    $badthings = TextHelper::getBasicNamingErrors($myvalues['item_nm']);
                    foreach($badthings as $onebad)
                    {
                        form_set_error('item_nm', 'The name "' . $myvalues['item_nm'] . '"' . $onebad);
                        $bGood = FALSE;
                    }
    ...
    function getForm($formType, $form, &$form_state, $disabled, $myvalues, $html_classname_overrides=NULL)
    {
        try
        {
            ...
            $intrashcan_ynoptions = array(
                0 => t('Yes'),
                1 => t('No')
            );
            $is_in_trashcan = isset($myvalues['active_yn']) ? !$myvalues['active_yn'] : FALSE;
            $form['data_entry_area1']['active_yn'] = array(
                '#type' => 'radios',
                '#title' => t('In Trashcan'),
                '#default_value' => isset($myvalues['active_yn']) ? $myvalues['active_yn'] : 1,
                '#options' => $intrashcan_ynoptions,
                '#description' => t('Yes if workitem is in the trashcan ready to be permanently discarded.')
            );
            if(!$disabled || !$is_in_trashcan)
            {
                ...
                if(!$disabled)
                {
                    $form['data_entry_area1']['parkinglot_level']['#states'] = array(
                        'visible' => array(   // action to take.
                          ':input[name=active_yn]' => array('value' => 1),
                        ),
                      );                
