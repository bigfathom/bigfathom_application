 ...
 */
class CreateTemplateFromProjectPageHelper
{
    ...
    function getFieldValues($projectid=NULL)
    {
        try
        {
            ...
            if(!empty($projectid))
            {
                ...
                $new_template_nm = $this->m_oMapHelper->getNewTemplateProjectName($projectid);
                $myvalues['original_root_workitem_nm'] = $myvalues['root_workitem_nm'];
                $myvalues['template_nm'] = $new_template_nm;
    ...
    function formIsValid($form, &$myvalues, $formType)
    {
        try
        {
            ...
            if($bGood)
            {
                $proposed_name = $this->m_oMapHelper->getNewTemplateProjectName($source_projectid, $new_root_workitem_nm);
    ...
    function getForm($formType, $form, &$form_state, $disabled, $myvalues, $html_classname_overrides=NULL)
    {
        try
        {
            ...
            if(isset($myvalues['template_nm']))
            {
                ...
                $template_nm = "Template of " . $root_workitem_nm;
            }
            if(isset($myvalues['template_author_nm']))
            {
                $template_author_nm = $myvalues['template_author_nm'];
            } else {
                $template_author_nm = '';
            }
            ...
            $form['data_entry_area1']["blurb"]    = array(
                ...
                '#markup' => "<p>"
                . t("The template created from contents of project#{$this->m_this_projectid} will create a new template with duplicates of all its workitems.  The template will be saved with the values you set here.")
                . "</p>",
                '#suffix' => '</div>',
            );
            ...
            $form['data_entry_area1']['template_nm'] = array(
                ...
                '#maxlength' => 40,
                '#required' => TRUE,
                '#description' => t('A name for the template'),
                '#disabled' => FALSE
            );
            $form['data_entry_area1']['root_workitem_nm'] = array(
                ...
                '#maxlength' => 40,
                '#required' => TRUE,
                '#description' => t('The root goal name in the source project is "' . $original_root_workitem_nm .'".'),
                '#disabled' => FALSE
            );
            $form['data_entry_area1']['purpose_tx'] = array(
                ...
                '#maxlength' => 1024,
                '#required' => TRUE,
                '#description' => t('Short description of the specific purpose addressed by the root goal of this template'),
                '#disabled' => FALSE
            );
            ...
            $form['data_entry_area1']['mission_tx'] = array(
                ...
                '#maxlength' => 1024,
                '#required' => TRUE,
                '#description' => t('The overall mission vision of the project represented by this template.  A mission context statement should suggest a bigger aspiration area to which successfully completing the root goal of this project is relevant.'),
                '#disabled' => FALSE
            );

            $form['data_entry_area1']['template_author_nm'] = array(
                '#type' => 'textfield',
                '#title' => "<i class='fa fa-male' aria-hidden='true'></i> " . t('Template Author'),
                '#default_value' => $template_author_nm,
                '#size' => 80,
                '#maxlength' => 80,
                '#required' => TRUE,
                '#description' => t('The name of the template author'),
                '#disabled' => FALSE
            );
            
            $form['data_entry_area1']['submitter_blurb_tx'] = array(
                '#type' => 'textarea',
                '#title' => t('Submitter Overview Blurb'),
                '#default_value' => $submitter_blurb_tx,
                '#size' => 80,
                '#maxlength' => 1024,
                '#required' => TRUE,
                '#description' => t('Provide any additional overall information that is not already part of the mission statement for the template you are creating.'
                        . '  For example, you can capture here nuances of the relevant scenarious to which this template will best apply.'),
                '#disabled' => FALSE
            );
            ...
            $form['data_entry_area1']['owner_personid'] = array(
                ...
                '#required' => TRUE,
                '#description' => t('Who is directly responsible for the creation/maintenance of this template?'),
                '#disabled' => FALSE
            );
