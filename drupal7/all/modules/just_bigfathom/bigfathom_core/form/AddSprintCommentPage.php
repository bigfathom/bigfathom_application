...
/**
 * Add a Sprint Comment
 *
 * @author Frank Font
 */
class AddSprintCommentPage extends \bigfathom\ASimpleFormPage
{
    protected $m_projectid          = NULL;
    protected $m_sprintid           = NULL;
    protected $m_comid              = NULL;
    protected $m_comment_type       = NULL;
    protected $m_parent_sprintid      = NULL;
    ...
    {
//DebugHelper::showNeatMarkup($urls_arr,'LOOK SPRINT URLS IN');        
        if($parentkey == NULL || !is_array($parentkey))
        {
            throw new \Exception("Cannot add a sprint without specifying a parent key!");
        }
        ...
        if(array_key_exists('parent_comid', $parentkey))
        {
            ...
            $record = $this->m_oMapHelper->getSprintCommunicationContext($this->m_parent_comid);
            $this->m_projectid = $record['projectid'];
            $this->m_sprintid = $record['sprintid'];
            $this->m_comment_type = 'REPLY';
        } else
        if(array_key_exists('sprintid', $parentkey))
        {
            $this->m_comid = NULL;
            $this->m_sprintid = $parentkey['sprintid'];
            $this->m_projectid = $this->m_oMapHelper->getProjectIDForSprint($this->m_sprintid);
    ...
    {
        $values = $this->m_oPageHelper->getCommentFieldValues(NULL,$this->m_parent_comid,$this->m_sprintid);
        return $values;
    }
    ...
    function updateDatabase($form, $myvalues)
    {
        try
        {
            $resultbundle = $this->m_oPageHelper->createSprintComment($myvalues);
            drupal_set_message($resultbundle['message']);
        }
        catch(\Exception $ex)
        {
            $msg = t('Failed to add sprint comment because ' . $ex->getMessage());
            error_log("$msg\n" 
    ...
    function getForm($form, &$form_state, $disabled, $myvalues_override)
    {
        ...
        $myvalues['sprintid'] = $this->m_sprintid;
        ...
        $new_form['data_entry_area1']['action_buttons']['create'] = array('#type' => 'submit'
                ...
                , '#value' => t('Save Sprint Communication'));
        ...
        if(isset($this->m_urls_arr['return']))
        {
            ...
            $sReturnMarkup = l('Cancel',$returnURL, 
                        array(
                            'query' => array('sprintid' => $this->m_sprintid,'cpk'=>$this->m_custom_page_key),
