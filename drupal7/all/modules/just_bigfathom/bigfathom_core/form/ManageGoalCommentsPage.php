 ...
 */
class ManageGoalCommentsPage extends \bigfathom\ASimpleFormPage {
    ...
    public function __construct($goalid=NULL, $urls_override_arr=NULL)
    {
        if(empty($goalid))
        {
            throw new \Exception("Cannot find comments without a goalid!");
        }
        ...
        $this->m_oPageHelper = new \bigfathom\GoalPageHelper($urls_arr, NULL, $this->m_parent_projectid);
    ...
    function getForm($form, &$form_state, $disabled, $myvalues, $html_classname_overrides = NULL) {
        try 
        {
            $main_tablename = 'goalcomment-table';
            ...
            $dashboard = $this->m_oPageHelper->getContextDashboardElements($this->m_goalid);
            ...
            $goalid=$this->m_goalid;
            $allcommentsmap = $this->m_oMapHelper->getWorkitemCommunications($goalid);
            ...
            foreach ($naturalorder_ar as $comid) 
            {
                ...
                {
                    $status_record = $goal_status_by_code[$status_cd_at_time_of_com];
            ...
            $form["data_entry_area1"]['table_container']['maintable'] = array('#type' => 'item',
                ...
                . '<th><span title="Goal status at time of comment creation">' . t('Goal Status') . '</span></th>'
            ...
            if (isset($this->m_urls_arr['add']) && $this->m_parent_projectid != NULL)
            {
                if (strpos($this->m_aPersonRights, 'A') !== FALSE) {
                    $add_link_markup = l('Add New Goal Comment Thread'
                            , $this->m_urls_arr['add']
                            , array('query' => array('goalid' => $this->m_goalid),
