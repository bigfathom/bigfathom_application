 ...
 * @author Frank Font
 */
class DeleteGroupPage extends \bigfathom\ASimpleFormPage
{
    protected $m_group_id       = NULL;
    ...
    function __construct($groupid)
    {
        ...
        {
            global $user;
            error_log("HACKING WARNING: uid#{$user->uid} attempted to delete groupid#$groupid!!!");
            throw new \Exception("Illegal access attempt!");
        }
        if($groupid == Context::$SPECIALGROUPID_DEFAULT_PRIVCOLABS 
                || $groupid == Context::$SPECIALGROUPID_EVERYONE 
                || $groupid == Context::$SPECIALGROUPID_NOBODY)
        {
            global $user;
            error_log("HACKING WARNING: uid#{$user->uid} attempted to delete the protected groupid#$groupid!!!");
            throw new \Exception("Illegal access attempt!");
        }
    }
    ...
    function updateDatabase($form, $myvalues)
    {
        ...
        {
            $groupid = $myvalues['id'];
            db_delete($this->m_group_tablename)
              ->condition('id', $groupid)
              ->execute(); 
            //If we are here then we had success.
            $msg = 'Deleted group ' . $myvalues['group_nm'];
            drupal_set_message($msg);
        }
        catch(\Exception $ex)
        {
            $msg = t('Failed to delete ' . $myvalues['group_nm']
    ...
    function getForm($form, &$form_state, $disabled, $myvalues, $html_classname_overrides=NULL)
    {
        ...
        $buttontext = 'Delete Group From System';
