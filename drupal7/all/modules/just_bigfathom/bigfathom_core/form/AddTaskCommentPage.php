 ...
 * @author Frank Font
 */
class AddTaskCommentPage extends \bigfathom\ASimpleFormPage
{
    ...
    protected $m_task_tablename = 'bigfathom_workitem';
    ...
    public function __construct($parentkey, $urls_arr=NULL)
    {
        ...
        {
            throw new \Exception("Cannot add a task without specifying a parent key!");
        }
        ...
        if(array_key_exists('taskid', $parentkey))
        {
            $this->m_comid = NULL;
            $this->m_taskid = $parentkey['taskid'];
            $this->m_projectid = $this->m_oMapHelper->getProjectIDForTask($this->m_taskid);
    ...
    {
        $values = $this->m_oPageHelper->getCommentFieldValues(NULL,$this->m_parent_comid,$this->m_taskid);
        return $values;
    }
    ...
    function updateDatabase($form, $myvalues)
    {
        try
        {
            $resultbundle = $this->m_oPageHelper->createTaskComment($myvalues);
            drupal_set_message($resultbundle['message']);
        }
        catch(\Exception $ex)
        {
            $msg = t('Failed to add task comment because ' . $ex->getMessage());
            error_log("$msg\n" 
    ...
    function getForm($form, &$form_state, $disabled, $myvalues_override)
    {
        ...
        $new_form['data_entry_area1']['action_buttons']['create'] = array('#type' => 'submit'
                ...
                , '#value' => t('Add Task Communication'));
