 ...
 */
class ManageWorkitemCommentsPage extends \bigfathom\ASimpleFormPage 
{
    ...
    public function __construct($workitemid=NULL, $urls_override_arr=NULL)
    {
        ...
        if(empty($workitemid))
        {
            throw new \Exception("Cannot find comments without a workitemid!");
        }
        ...
        $urls_arr = [];
        $urls_arr['add'] = 'bigfathom/workitem/addcomment';
        $urls_arr['reply'] = 'bigfathom/workitem/addcomment';
        $urls_arr['edit'] = 'bigfathom/workitem/editcomment';
        $urls_arr['view'] = 'bigfathom/workitem/viewcomment';
        $urls_arr['delete'] = 'bigfathom/workitem/deletecomment';
    ...
    function getForm($form, &$form_state, $disabled, $myvalues, $html_classname_overrides = NULL) 
    {
        try 
        {
            $main_tablename = 'workitemcomment-table';
            ...
            $dashboard = $this->m_oPageHelper->getContextDashboardElements($this->m_workitemid);
            ...
            $workitemid=$this->m_workitemid;
            $allcommentsmap = $this->m_oMapHelper->getWorkitemCommunications($workitemid);
            ...
            foreach ($naturalorder_ar as $comid) 
            {
                ...
                {
                    $status_record = $workitem_status_by_code[$status_cd_at_time_of_com];
            ...
            $form["data_entry_area1"]['table_container']['maintable'] = array('#type' => 'item',
                ...
                . '<th><span title="Workitem status at time of comment creation">' . t('Workitem Status') . '</span></th>'
            ...
            if (isset($this->m_urls_arr['add']) && $this->m_parent_projectid != NULL)
            {
                if (strpos($this->m_aPersonRights, 'A') !== FALSE) {
                    $add_link_markup = l('Add New Workitem Comment Thread'
                            , $this->m_urls_arr['add']
                            , array('query' => array('workitemid' => $this->m_workitemid,'cpk'=>$this->m_custom_page_key),
