...
/**
 * This class returns the list of available locations
 *
 * @author Frank Font of Room4me.com Software LLC
 */
class ManageLocationPage extends \bigfathom\ASimpleFormPage
{
    ...
    public function __construct()
    {
        ...
        $urls_arr = array();
        $urls_arr['add'] = 'bigfathom/addlocation';
        $urls_arr['edit'] = 'bigfathom/editlocation';
        $urls_arr['view'] = 'bigfathom/viewlocation';
        $urls_arr['delete'] = 'bigfathom/deletelocation';
    ...
    function getForm($form, &$form_state, $disabled, $myvalues, $html_classname_overrides=NULL)
    {
        try
        {
            ...
            foreach($all as $locationid=>$record)
            {
                
                $shortname = $record['shortname'];
                $shortname_markup = "<span title='#$locationid'>$shortname</span>";
                ...
                $address_lines_markup = \bigfathom\MarkupHelper::getAddressMarkup($address_line1,$address_line2,$city_tx,$state_abbr,$country_abbr);
                ...
                if(strpos($this->m_aDataRights,'V') === FALSE || !isset($this->m_urls_arr['view']))
                {
                    ...
                    //$sViewMarkup = l('View',$this->m_urls_arr['view'],array('query'=>array('locationid'=>$locationid)));
                    $view_page_url = url($this->m_urls_arr['view'], array('query'=>array('locationid'=>$locationid)));
                    $view_icon_url = \bigfathom\UtilityGeneralFormulas::getIconURLForPurposeName('view');
                    $sViewMarkup = "<a title='view #{$locationid}' href='$view_page_url'><img src='$view_icon_url'/></a>";
                }
                if(strpos($this->m_aDataRights,'E') === FALSE || !isset($this->m_urls_arr['edit']))
                {
                    $sEditMarkup = '';
                } else {
                    //$sEditMarkup = l('Edit',$this->m_urls_arr['edit'],array('query'=>array('locationid'=>$locationid)));
                    $edit_page_url = url($this->m_urls_arr['edit'], array('query'=>array('locationid'=>$locationid)));
                    $edit_icon_url = \bigfathom\UtilityGeneralFormulas::getIconURLForPurposeName('edit');
                    $sEditMarkup = "<a title='edit #{$locationid}' href='$edit_page_url'><img src='$edit_icon_url'/></a>";
                }
                if(strpos($this->m_aDataRights,'D') === FALSE || !isset($this->m_urls_arr['delete']))
                {
                    $sDeleteMarkup = '';
                } else {
                    //$sDeleteMarkup = l('Delete',$this->m_urls_arr['delete'],array('query'=>array('locationid'=>$locationid)));
                    $delete_page_url = url($this->m_urls_arr['delete'], array('query'=>array('locationid'=>$locationid)));
                    $delete_icon_url = \bigfathom\UtilityGeneralFormulas::getIconURLForPurposeName('delete');
                    $sDeleteMarkup = "<a title='delete #{$locationid}' href='$delete_page_url'><img src='$delete_icon_url'/></a>";
                }
                $rows   .= "\n".'<tr><td>'
            ...
            $form["data_entry_area1"]['table_container']['ci'] = array('#type' => 'item',
                    '#markup' => '<table id="' . $main_tablename . '" class="browserGrid">'
                                . '<thead>'
                                . '<tr>'
                                . '<th title="A unique short name for this location">'.t('Shortname').'</th>'
                                . '<th>'.t('Address').'</th>'
                                . '<th title="Short description of this location">'.t('Description').'</th>'
                                . '<th title="Is this still an accurate location for references to use?">'.t('Accurate').'</th>'
            ...
            if(isset($this->m_urls_arr['add']))
            {
                ...
                {
                    $initial_button_markup = l('ICON_ADD Add New Location',$this->m_urls_arr['add']
