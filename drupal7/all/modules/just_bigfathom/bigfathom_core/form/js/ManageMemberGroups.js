...
bigfathom_util.table.showTotals = function(controller, minmaxinfo) 
{
    ...
    {
        markup = "<div class='grid-totals-container'>There are no available groups.</div>";
    } else {
        ...
        var count_markup = "<fieldset class='elem-inline'><legend title='Count of groups'>Count Totals</legend><div class='group-standard'>" 
                + selectedcount_elem
                + " "
...
//alert("LOOK we called setOneRowMembership for rowIndex=" + rowIndex);    
    var groupid = controller.getRowId(rowIndex);
...
{ 
    ...
    console.log("LOOK groupid=" + groupid);
    var change_comment;
    var values = controller.getRowValues(rowIndex);
    
    //Update this one row with other data
    var send_dataname = 'update_project_group_membership';
    var send_fullurl = bigfathom_util.data.getSendDataUrl(send_dataname);
    var ismember = newValue;
    change_comment = "grid edit projectid#" + projectid + " changed has groupid#" + groupid + " from '" + oldValue + "' to '" + newValue + "'";
    var databundle = {  
                        "projectid": projectid,
    ...
    if(ismember)
    {
        databundle['new'] = [groupid];    
    } else {
        databundle['remove'] = [groupid];    
    }
    ...
    var callbackActionFunction = function(callbackid, responseBundle)
    {
        ...
        uiblocker.hide("tr#" + groupid);
    };
    uiblocker.show("tr#" + groupid);
    bigfathom_util.data.writeData2Server(send_fullurl, sendpackage, callbackActionFunction, groupid);
