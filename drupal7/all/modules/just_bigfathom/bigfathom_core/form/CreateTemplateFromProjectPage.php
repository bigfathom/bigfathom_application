...
require_once 'helper/CreateTemplateFromProjectPageHelper.php';
 ...
 * @author Frank Font
 */
class CreateTemplateFromProjectPage extends \bigfathom\ASimpleFormPage
{
    ...
    function __construct($projectid, $urls_override_arr=NULL)
    {
        ...
        $this->m_oPageHelper = new \bigfathom\CreateTemplateFromProjectPageHelper($urls_arr,NULL,NULL,$projectid);
    }
    ...
    function updateDatabase($form, $myvalues)
    {
        try
        {
            ...
            $template_projectid = $myvalues['id'];

            $result_bundle = $this->m_oWriteHelper->createTemplateFromProject($template_projectid, $myvalues);
            //If we are here then we had success.
            $msg = 'Created project template#' . $result_bundle['template_projectid'] . ' with name "' . $result_bundle['template_nm'] . '"';
            drupal_set_message($msg);
        }
        catch(\Exception $ex)
        {
            $msg = t('Failed to create template from project#' . $myvalues['id']
            ...
            , $html_classname_overrides=NULL)
    {
        ...
        $form['data_entry_area1']['action_buttons']['create'] = array(
            ...
            '#value' => t('Create Project Template'),
            '#disabled' => FALSE
        );
