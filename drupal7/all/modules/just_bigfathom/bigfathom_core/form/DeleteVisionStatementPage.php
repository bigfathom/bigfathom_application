...
/**
 * Delete one vision statement
 *
 * @author Frank Font
 */
class DeleteVisionStatementPage extends \bigfathom\ASimpleFormPage
{
    ...
    function __construct($visionstatementid)
    {
        ...
        {
            global $user;
            error_log("HACKING WARNING: uid#{$user->uid} attempted to delete visionstatementid#$visionstatementid!!!");
    ...
    function updateDatabase($form, $myvalues)
    {
        try
        {
            ...
            db_delete(DatabaseNamesHelper::$m_visionstatement_tablename)
              ->condition('id', $visionstatementid)
              ->execute(); 
            //If we are here then we had success.
            $msg = 'Deleted statement ' . $myvalues['statement_nm'];
            drupal_set_message($msg);
        }
        catch(\Exception $ex)
        {
            $msg = t('Failed to delete ' . $myvalues['statement_nm']
    ...
    function getForm($form, &$form_state, $disabled, $myvalues, $html_classname_overrides=NULL)
    {
        ...
        $buttontext = 'Delete Vision Statement From System';
