(function ($) {
  ...
  {
    //$('#'+ sid +' option:selected', context).attr("selected", "");    NO GOOD
    //$('#'+ sid +' option:selected', context).removeAttr("selected");  NO GOOD
    var now_selected_values = {};   //FJF 20170628 fix for NASTY bug that removed items not detected by Drupal on submit of JS element
    $('#improvedselect-'+ sid +' .improvedselect_sel li', context).each(function(){
        var the_value = $(this).attr('so');
        now_selected_values[the_value] = the_value;
        $('#'+ sid +' option[value="'+ $(this).attr('so') +'"]', context).attr("selected", "selected").change();
        $('#'+ sid +' [value="'+ $(this).attr('so') +'"]', context).attr("selected", "selected");
    });

    $('#' + sid, context).find('option').each(function() {
      var the_value = $(this).attr('value');
      if(now_selected_values.hasOwnProperty(the_value))  //($(this).attr("selected")) 
      {
        $('#improvedselect-'+ sid +' .improvedselect_sel', context).append($('#improvedselect-'+ sid +' .improvedselect_sel [so="'+ the_value +'"]', context));
      }
      else 
      {
        $('#improvedselect-'+ sid +' .improvedselect_all', context).append($('#improvedselect-'+ sid +' .improvedselect_all [so="'+ the_value +'"]', context));
        $('#'+ sid +' option[value="'+ the_value +'"]', context).removeAttr("selected");    //FJF 20170628 part of the fix
      }
    });
  }
